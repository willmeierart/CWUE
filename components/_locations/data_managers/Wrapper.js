	...
	class WrappedComponent extends Component {
		...
		componentDidMount () {
			...
			if (this.props.swAvailable) {
				...
				// console.log('WORKER AVAILABLE', this.geoWorker)
			}
		}
		...
		componentDidUpdate (prevProps) {
			if (this.props.activeLocation !== prevProps.activeLocation || this.props.url !== prevProps.url) {
		...
		setMapZoomModifier (mapZoomModifier) {
			// to more precisely handle zooming when only one result present
			...
			switch (true) {
				case initial:
					if (initial && url.asPath !== '/carwash/locations') {
						// if it's supposed to be the initial view but it isn't, make it so
		...
		setPageStateGeoLoc (state) {
			// determines whether initial view should be 'initial' or results near user's location, if that's available
			...
			if (userLocation !== null && userLocation !== 'denied' && userLocation !== 'unavailable' && userLocation !== {}) {
				if (!userIsLocated) {
					// if the user hasn't been located, then try again to locate them
