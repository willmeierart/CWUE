...
class MobileMenu extends Component {
  ...
  handleClick (route, hasChildren, child) {
    ...
    if (hasChildren) {
      ...
      if (route.page === 'locations') {
        ...
        ImperativeRouter.push(route.page, { title: formattedTitle }, false)
      }
  ...
  renderSubList (route) {
    ...
    const list = route.children.map((child, i) => {
      const { title } = child
      return (
        <li ref={el => { this[route.page] = el }} key={i} className='child-route' onClick={() => { this.handleClick(route, false, child) }}>
          <Link>
            <a>{ title }</a>
          </Link>
          <style jsx>{`
  ...
  renderList () {
    return routes.filter(route =>
      route.title.toLowerCase().indexOf('account') === -1 &&
      route.title.toLowerCase().indexOf('legal') === -1
    ).map((route, i) => {
      // const formattedTitle = route.title.toLowerCase().replace(' ', '-')
      const hasChildren = route.children && route.children.length > 0
      return (
        <li key={route.title} onClick={() => { this.handleClick(route, hasChildren, null) }}>
