...
function createClient (initialState, client) {
  const HttpLinkData = {
    uri: client === 'locations'
      ? TEST_LOCAL
        ? process.env.GRAPHQL_PROXY_LOCAL
        : process.env.GRAPHQL_PROXY // this is for the multi-endpoint woven-schema for the locations page
...
export default function initApollo (initialState, client) {
  ...
  if (!process.browser || client === 'locations') { // if we do run into problems down the road with the double-client provider, this is probably what's causing it...
